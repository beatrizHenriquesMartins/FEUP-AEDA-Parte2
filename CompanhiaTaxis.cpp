...
void CompanhiaTaxis::fazerViagemCliente(int id, Data dia, Hora horaIn, Percurso p1, bool disc, float per, string tipoPag) {
	...
	for (unsigned int j = 0; j < clientes.size(); j++) {
		if (clientes[j]->getID() == id) {
			...
			for (unsigned int i = 0; i < taxisTotais.size(); i++) {
				...
				if (taxisTotais[i].getdispo((horaIn, v.getHoraOut())) {
